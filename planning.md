...
I'll give Claude my Tool specs separately asking to implement search_listings first, then suggest_outfit, then create_fit_card with each tool's input return value and failure mode. Between each tool implementation, I'll test the tools. For search_listings, I'll test with multiple queries to make sure relevant results are given from load_listings in order or that a message is given is none can be found. For suggest_outfit, I'll test to make sure the example user's wardrobe input is used in the reponse and empty wardrobe returns a general styling response. For create_fit_card I'll make sure the response is a casual authentic caption or that an error message is given if there's no complete outfit input.
